...
@login_required
def dashboard():
    data_manager.clean_old_exams()
    halls = data_manager.get_halls()
    exams = data_manager.get_exams()
...
def classes():
    if request.method == 'POST':
        ...
        data_manager.add_class(department_id, name)
        flash('Class added successfully!', 'success')
        return redirect(url_for('classes'))
        
    cls = data_manager.get_classes()
    depts = data_manager.get_departments()
    
    # map dept id to name for display
...
def edit_class(class_id):
    cls = data_manager.get_class(class_id)
    if not cls:
    ...
    if request.method == 'POST':
        name = request.form['name']
        department_id = request.form['department_id']
        data_manager.update_class(class_id, name, department_id)
...
def delete_class(class_id):
    data_manager.delete_class(class_id)
...
def students():
    if request.method == 'POST':
        # Check if this is a CSV upload or a single student addition
        if 'csv_file' in request.files:
            ...
            if file.filename == '':
                ...
                count = data_manager.import_students_from_csv(file_stream, class_id)
    ...
    studs = data_manager.get_students()
    cls = data_manager.get_classes()
...
def exams():
    ...
    if request.method == 'POST':
        ...
        data_manager.add_exam(class_id, subject, date, start_time, end_time)
        flash('Exam scheduled successfully!', 'success')
        return redirect(url_for('exams'))
        
    exs = data_manager.get_exams()
    cls = data_manager.get_classes()
    allots = data_manager.get_allotments()
    
    cls_map = {c['id']: c['name'] for c in cls}
    
    # Check if exam has allotments
    allot_map = {}
    for a in allots:
...
def do_allotment(exam_id):
    success, message = data_manager.perform_allotment(exam_id)
    if success:
...
def view_allotment(exam_id):
    exam = data_manager.get_exam(exam_id)
    if not exam:
        flash('Exam not found!', 'danger')
        return redirect(url_for('exams'))
        
    cls = data_manager.get_class(exam['class_id'])
    exam['class_name'] = cls['name'] if cls else 'Unknown'
        
    allots = data_manager.get_allotments_for_exam(exam_id)
    halls = data_manager.get_halls()
    students = data_manager.get_students()
